...
class Simulation:
    ...
    def draw_player(self):
        ...
        self.win.blit(x_text, dest=[
            self.player.pos[0] - x_text.get_width() // 2 + (self.player.size // 2) * np.sin(
                np.deg2rad(270 + self.player.direction)),
            self.player.pos[1] - x_text.get_height() // 2 - (self.player.size // 2) * np.cos(
                np.deg2rad(270 + self.player.direction)),
        ])
        self.win.blit(y_text, dest=[
            self.player.pos[0] - y_text.get_width() // 2 + (self.player.size // 2) * np.sin(
                np.deg2rad(90 + self.player.direction)),
            self.player.pos[1] - y_text.get_height() // 2 - (self.player.size // 2) * np.cos(
                np.deg2rad(90 + self.player.direction)),
        ])
        ...
        for i in range(self.player.num_sensors):
            ...
            self.win.blit(text, dest=[
                self.player.pos[0] - text.get_width() // 2 + (self.player.size + 20) * np.sin(
                    np.deg2rad(i * angle + self.player.direction)),
                self.player.pos[1] - text.get_height() // 2 - (self.player.size + 20) * np.cos(
                    np.deg2rad(i * angle + self.player.direction)),
            ])
