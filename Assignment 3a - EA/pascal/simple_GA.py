...
def animation_visualizer(xss, yss, fitness, rosenbrock=True):
    num_particles = len(xss[0])

    best_fitness = [f[0] for f in fitness]
    avg_fitness = [sum(f)/len(f) for f in fitness]
    ...
    ax_all_perf = fig.add_subplot(gs[0:2, 2])
    ...
    ax_all_perf.set_xlim(0, len(xss)-1)
    ax_all_perf.set_ylim(-0.1, 50)
    ax_all_perf.set_title("Particle Fitness")
    ax_all_perf.set_xlabel("Iteration")
    ax_all_perf.set_ylabel("Fitness")
    line_all_data = [[], []]
    all_lines = [ax_all_perf.plot([], [], lw=2, alpha=0.8, label="Top Fitness")[0],
                 ax_all_perf.plot([], [], lw=2, alpha=0.8, label="Average Fitness")[0]]
    ax_all_perf.legend()
    ...
    def animate_all_perf(i, lines):
        line_all_data[0].append(best_fitness[i])
        lines[0].set_data([index for index, _ in enumerate(line_all_data[0])], line_all_data[0])
        line_all_data[1].append(avg_fitness[i])
        lines[1].set_data([index for index, _ in enumerate(line_all_data[1])], line_all_data[1])
        return lines
